...
| Method | Path                                                                   | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ------ | ---------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| GET    | `/subscribers?q=&list_ids=&list_statuses=&blocklisted=&limit=&offset=` | Returns `{ subscribers: Subscriber[], total: number }` — `total` is the count of rows matching the current filter (ignoring `limit`/`offset`), for pagination and "select all matching". `q` is a substring match on email and name only (not `attribs`); `list_ids` and `list_statuses` are comma-separated; `blocklisted=true` widens the result rather than narrowing it (it's OR'd with the list filter). `limit` defaults to `50`, max `200`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| GET    | `/subscribers/:id`                                                     | Includes list memberships                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| POST   | `/subscribers`                                                         | `{ email, name?, status?, attribs?, list_ids?, preconfirm? }` -- upserts by email. `status` is `enabled \| blocklisted` (default `enabled`). `preconfirm` marks the given `list_ids` as confirmed immediately instead of the opt-in-type default. A supplied `attribs` **replaces** an existing contact's whole attributes object -- see [Subscriber attributes](#subscriber-attributes-attribs) before using this to update an existing contact.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| PATCH  | `/subscribers/:id`                                                     | `{ name?, attribs? }`. `attribs` **replaces** the whole object -- see [Subscriber attributes](#subscriber-attributes-attribs).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
`attribs` is one free-form JSONB object per subscriber. Custom fields (`plan`,
`signup_source`, ...) and the contact's tags (a string array at `attribs.tags`,
written by the tag endpoints) all live in that single object, and campaign
templates read keys straight out of it (`{{ Subscriber.Attribs.plan }}`). Any
write that replaces the object wholesale therefore drops every key it doesn't
mention, tags included.

Only the import endpoint merges. Everything else replaces:

| Write                          | Effect on an existing contact's `attribs`                                                                                             |
| ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------- |
| `POST /subscribers`            | **Replaces** the whole object with the one you send                                                                                   |
| `PATCH /subscribers/:id`       | **Replaces** the whole object                                                                                                         |
| `POST /automations/hooks/:key` | **Replaces** the whole object, but only when the contact is resolved by `email`; `attribs` is ignored when you pass a `subscriber_id` |
| `POST /subscribers/import`     | Whatever `attribs_mode` says: `merge` (default), `replace`, or `skip`                                                                 |

Omitting `attribs` from the body leaves the stored object untouched on all of
them; sending `{}` to a replacing endpoint clears it.

So updating attributes on a contact who may already exist -- an app event
stamping `last_order_at`, say -- goes through the import endpoint with
`attribs_mode: "merge"`, not `POST /subscribers`:

```jsonc
// POST /api/v1/subscribers/import
{
  "attribs_mode": "merge", // default; keeps keys set by earlier events
  "overwrite_user_info": false, // default; don't clobber an existing name
  "list_ids": [], // no list changes -- attributes only
  "subscribers": [{ "email": "a@example.com", "attribs": { "last_order_at": "2026-08-31" } }],
}
```

With `list_ids: []` that touches nothing but the attributes, and it still
creates the contact (firing `contact_created`) when the address is new -- one
upsert-and-merge call per event. It needs the `subscribers:import` permission,
where `POST /subscribers` and `PATCH /subscribers/:id` need
`subscribers:manage`.

Two limits of `merge`: it is a shallow top-level JSONB merge (`attribs ||
incoming`), so a nested object under a key is swapped whole rather than
deep-merged; and it cannot remove a key -- setting one to `null` stores a JSON
null, and actually dropping keys means sending the full desired object through
`replace` or `PATCH /subscribers/:id`.

Worked example -- a contact holding `{ "plan": "pro", "tags": ["vip"] }`,
written with `{ "attribs": { "last_order_at": "2026-08-31" } }`:

- through `POST /subscribers` → `{ "last_order_at": "2026-08-31" }`; `plan` and
  the `vip` tag are gone
- through `POST /subscribers/import` with the default `merge` →
  `{ "plan": "pro", "tags": ["vip"], "last_order_at": "2026-08-31" }`
...
| GET    | `/automations/:id/analytics`   | `{ unsubscribes, unique_unsubscribes, reasons }` — departures attributed to this automation's emails, plus the reason breakdown. No admin UI consumes this endpoint yet                                                                                           |
